...
class Connexion{
    ...
    static updateServerAccess(parameters){
        ...
        if(Connexion.getCookie("secure_token")){
            Connexion.SECURE_TOKEN = Connexion.getCookie("secure_token");
        }
        ...
        if(parameters.get('SERVER_PREFIX_URI')){
            ...
            serverAccessPath = parameters.get('SERVER_PREFIX_URI') + serverAccessPath;
        }else{
