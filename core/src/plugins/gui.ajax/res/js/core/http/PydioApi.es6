...
class PydioApi{
    ...
    downloadSelection(userSelection, dlActionName='download', additionalParameters = {}){
        ...
        if(agentIsMobile || !hiddenForm){
            let downloadUrl = ajxpServerAccess;
            if(downloadUrl.includes("?")) {
                downloadUrl += '&get_action=' + dlActionName;
            }
            else {
                downloadUrl += '?get_action=' + dlActionName; 
            }
            ...
            let parameters = {...additionalParameters, get_action: dlActionName};
    ...
    buildUserAvatarUrl(userId, avatarId = null){
        ...
        if(baseURL.includes("?")) {
            baseURL += "&get_action=get_binary_param&binary_id=" + avatarId + "&user_id=" + userId;
        }
        else {
            baseURL += "?get_action=get_binary_param&binary_id=" + avatarId + "&user_id=" + userId;
        }
