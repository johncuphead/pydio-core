		...
		for(let i=0;i<repoNode.childNodes.length;i++){
            ...
            if(childNode.nodeName == "label"){
                ...
                if(childNode.getAttribute('icon_tpl_id')){
					var baseURL = window.pydio.Parameters.get('serverAccessPath');
					if(baseURL.includes("?")) {
						baseURL += '&get_action=get_user_template_logo&template_id='+childNode.getAttribute('icon_tpl_id')+'&icon_format=small';
					}
					else {
						baseURL += '?get_action=get_user_template_logo&template_id='+childNode.getAttribute('icon_tpl_id')+'&icon_format=small';
					}
                    this.setIcon(baseURL);
                }else{
