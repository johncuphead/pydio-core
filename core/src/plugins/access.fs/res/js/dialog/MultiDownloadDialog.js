...
let MultiDownloadDialog = React.createClass({
    ...
    render: function(){
        ...
        let chunkAction;
        if(!this.props.buildChunks){
            const baseUrl = this.props.pydio.Parameters.get('ajxpServerAccess');
            if(baseUrl.includes("?")) {
                baseUrl += '&get_action='+this.props.actionName+'&file=';
            }
            else {
                baseUrl += '?get_action='+this.props.actionName+'&file=';
            }
            ...
            const baseUrl = this.props.pydio.Parameters.get('ajxpServerAccess');
            if(baseUrl.includes("?")) {
                baseUrl += '&get_action='+this.props.actionName+'&file_id=' + chunkData.file_id;
            }
            else {
                baseUrl += '?get_action='+this.props.actionName+'&file_id=' + chunkData.file_id;
            }
