        ...
        PydioApi.getClient().request({get_action:'get_upgrade_path'}, function(transp){
            ...
            if(response && response.packages.length){
                ...
                if(response.latest_note){
                    ...
                    latest = pydio.Parameters.get('ajxpServerAccess');
                    if(latest.includes("?")) {
                        latest +=  '&get_action=display_upgrade_note';
                    }
                    else {
                        latest +=  '?get_action=display_upgrade_note';
                    }
    ...
    performUpgrade: function(){
        ...
        if(confirm(this.context.getMessage('15', 'updater'))){
            ...
            this.setState({src:''}, function(){
                this.setState({src: client._baseUrl + '?get_action=perform_upgrade&package_index=' + this.state.checks});
            }.bind(this));
