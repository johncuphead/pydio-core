...
class Editor extends React.Component {
    ...
    loadNode(props) {
        ...
        if(baseURL.includes("?")) {
            baseURL += "&get_action=post_to_server&file=base64encoded:" + HasherUtils.base64_encode(node.getPath()) + "&parent_url=" + HasherUtils.base64_encode(DOMUtils.getUrlFromBase());
        }
        else {
            baseURL += "?get_action=post_to_server&file=base64encoded:" + HasherUtils.base64_encode(node.getPath()) + "&parent_url=" + HasherUtils.base64_encode(DOMUtils.getUrlFromBase());
        }
