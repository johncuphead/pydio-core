...
class ConfigLogo extends React.Component{
    ...
    render(){
        const pluginName = this.props.pluginName || 'gui.ajax';
        const pluginParameter = this.props.pluginParameter || 'CUSTOM_MINISITE_LOGO';
        ...
        if(logo){
            if(logo.indexOf('plugins/') === 0){
                ...
                if(url.includes("?")) {
                    url += "&get_action=get_global_binary_param&binary_id=" + logo;
                }
                else {
                    url += "?get_action=get_global_binary_param&binary_id=" + logo;
                }
                ...
                <MaterialUI.Paper zDepth={1} rounded={false} style={styles.appBarStyle}>
                    ...
                    <div style={{flex:1, position:'relative'}}>
                        ...
                        <div id="main_toolbar" style={{display:'flex', padding: '0 8px'}}>
                            ...
                            <Toolbar {...this.props} id="main-toolbar" toolbars={uniqueNode ? ["minisite_toolbar"] : ["info_panel", "minisite_folder"]} groupOtherList={uniqueNode ? [] : ["change_main", "more", "change", "remote"]} renderingType="button" buttonStyle={styles.buttonsStyle}/>
